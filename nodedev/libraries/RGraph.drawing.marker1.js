...
co.beginPath();if(prop['chart.shadow']){RG.SetShadow(this,prop['chart.shadow.color'],prop['chart.shadow.offsetx'],prop['chart.shadow.offsety'],prop['chart.shadow.blur']);}
co.lineWidth=prop['chart.linewidth'];co.strokeStyle=prop['chart.strokestyle'];co.fillStyle=prop['chart.fillstyle'];this.DrawMarker();co.stroke();co.fill();RG.NoShadow(this);co.fillStyle=prop['chart.text.color'];RG.Text2(this,{'font':prop['chart.text.font'],'size':prop['chart.text.size'],'x':this.coords[0][0]-1,'y':this.coords[0][1]-1,'text':this.text,'valign':'center','halign':'center','tag':'labels'});RG.InstallEventListeners(this);RG.FireCustomEvent(this,'ondraw');return this;}
...
{if(prop['chart.tooltips.highlight']){co.beginPath();co.strokeStyle=prop['chart.highlight.stroke'];co.fillStyle=prop['chart.highlight.fill'];this.DrawMarker();co.closePath();co.stroke();co.fill();}}
this.DrawMarker=function()
{var r=this.radius;if(prop['chart.align']=='left'){var x=this.markerCenterx;var y=this.markerCentery;co.arc(x,y,r,HALFPI,TWOPI,false);if(ISOLD){co.moveTo(x+r+r,y+r+r);co.quadraticCurveTo(x+r,y+r,x+r+1,y);co.moveTo(x+r+r,y+r+r);}else{co.quadraticCurveTo(x+r,y+r,x+r+r,y+r+r);}
co.quadraticCurveTo(x+r,y+r,x,y+r+(ISOLD?1:0));}else if(prop['chart.align']=='right'){var x=this.markerCenterx;var y=this.markerCentery;co.arc(x,y,r,HALFPI,PI,true);if(ISOLD){co.moveTo(x-r-r,y+r+r);co.quadraticCurveTo(x-r,y+r,x-r-1,y);co.moveTo(x-r-r,y+r+r);}else{co.quadraticCurveTo(x-r,y+r,x-r-r,y+r+r);}
co.quadraticCurveTo(x-r,y+r,x,y+r+(ISOLD?1:0));}else{var x=this.markerCenterx;var y=this.markerCentery;co.arc(x,y,r,HALFPI/2,PI-(HALFPI/2),true);if(ISOLD){co.moveTo(x,y+r+r-2);co.quadraticCurveTo(x,y+r+(r/4),x-(Math.cos(HALFPI/2)*r),y+(Math.sin(HALFPI/2)*r));co.moveTo(x,y+r+r-2);}else{co.quadraticCurveTo(x,y+r+(r/4),x,y+r+r-2);}
co.quadraticCurveTo(x,y+r+(r/4),x+(Math.cos(HALFPI/2)*r),y+(Math.sin(HALFPI/2)*r));}
this.coords[0]=[x,y,r];}
